...
export function StatusBadge({ status, className = "" }: { status: string; className?: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center font-label-sm text-label-sm font-semibold px-2.5 py-1 rounded-md border shadow-[inset_2px_2px_4px_rgba(23,32,51,0.1)] ${style} ${className}`}
    >
      {label}
    </span>
  );
